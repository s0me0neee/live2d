...
- `face-tracking.ts` — webcam → MediaPipe FaceLandmarker (blendshapes + transform matrix) → a `Rig` of Live2D params. Inference is throttled to `config.detectFps` independent of the render loop; smoothing runs per render frame. Writes face params on `afterMotionUpdate` and body/hair/cloth params on `beforeModelUpdate` so they win over (or feed) the physics sim. Reaches into Cubism runtime internals (`internalModel.coreModel`, `setParameterValueById`).
