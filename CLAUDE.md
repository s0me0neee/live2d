...
- **Config dir**: dev (`!app.isPackaged`) = `./config/web2d` under the project; packaged = `<appData>/web2d`. The model/tuning TOMLs are **tracked**; only `local.toml` (per-machine volatile state) is gitignored. Files seed on first run.
- **`config.toml`** — model-independent knobs, merged over `DEFAULT_CONFIG`, plus `model` = the active model name.
- **`models/<name>.toml`** — everything model-dependent in one file: `location`/`model` (both **required**; an unknown/typo'd active model loads nothing and **no file is created**), a `[gain]` table, and `[expressions.<name>]` tables (`file`/`key`/`active`). First load centers the model at scale 1; the live transform persists to `local.toml`'s `[pos.<model>]` (not here — keeps the tracked model TOML free of per-machine drag/zoom state). `DEFAULT_MODEL_CONFIG` is intentionally **blank**.
...
- Overlay window geometry + live model transform live in `local.toml` (`[window]` and `[pos.<model>]`), read/written by the sync `*LocalStateSync` helpers in `electron/config.ts`. `local.toml` is **gitignored** (per-machine, churns every session); `loadConfig`/`loadModel` migrate any stale `[window]`/`[pos]` out of the tracked TOMLs.
- `config/web2d/` — tracked model/tuning TOMLs; `local.toml` (window + pos) is gitignored.
